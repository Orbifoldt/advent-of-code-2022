The key here was finding the correct heuristic for comparing states in order to prune them. It turns out that we should first compare the number of geode robots, then the number of geodes, then the number of obsidian robots, then the number of obsidian etc.

I tried my very best to be smarter than the Kotlin compiler and the JVM, but ofcourse I failed :D I initially ran into memory issues when using data classes, so I tried using an IntArray instead, but that gave no noticeable improvement. What did help somewhat was using a queue in the BFS instead of just reading from a list, because at the same time I was making a new list with new states. Also, I converted my IntArrays to ShortArrays, which helped too. But still it was never going to fit all in my memory, hence the pruning.

Some other nice thing I found later on internet was checking whether creating a robot for some type made any sense: since we can create at most one robot per minute it would never make sense to gather more resources of some type than what we can spend per minute. 

Output:
```
Blueprint 1 found 9 geodes
Blueprint 2 found 12 geodes
For example-blueprints.txt the summed quality level is '33'
Blueprint 1 found 0 geodes
Blueprint 2 found 3 geodes
...
Blueprint 29 found 1 geodes
Blueprint 30 found 12 geodes
For my-blueprints.txt the summed quality level is '1725'
Blueprint 1 found 56 geodes
Blueprint 2 found 62 geodes
For example-blueprints.txt the product of the quality levels after 32 is '3472'
Blueprint 1 found 10 geodes
Blueprint 2 found 33 geodes
Blueprint 3 found 47 geodes
For my-blueprints.txt the product of the quality levels after 32 is '15510'
```
